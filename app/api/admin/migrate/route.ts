...
// Función para procesar un establecimiento y sus contactos
async function processEstablishment(establishment: any, contacts: any[]) {
  try {
    ...
    if (cue) {
      ...
      if (!checkError && existingEstablishment) {
        ...
        const transformedEstablishment: any = {
          ...
          // Información de predio y administración
          ...
          // Campos adicionales que puedan existir en el spreadsheet
          // Aseguramos que todos los campos se migren correctamente
          ...Object.keys(establishment)
            .filter(
              (key) =>
                !key.startsWith("_") && key !== "CUE" && !transformedEstablishment.hasOwnProperty(key.toLowerCase()),
            )
            .reduce((obj, key) => {
              const lowerKey = key.toLowerCase()
              obj[lowerKey] = establishment[key] || null
              return obj
            }, {}),
        }
    ...
    const transformedEstablishment: any = {
      ...
      ...Object.keys(establishment)
        .filter(
          (key) => !key.startsWith("_") && key !== "CUE" && !transformedEstablishment.hasOwnProperty(key.toLowerCase()),
        )
        .reduce((obj, key) => {
          const lowerKey = key.toLowerCase()
          obj[lowerKey] = establishment[key] || null
    ...
    if (establishmentError) {
      throw new Error(`Error al insertar establecimiento ${cue}: ${establishmentError.message}`)
    }
  ...
  for (const contact of relatedContacts) {
    try {
      const transformedContact: any = {
        ...
        // Aseguramos que todos los campos adicionales se migren correctamente
        ...Object.keys(contact)
          .filter(
            (key) =>
              !key.startsWith("_") &&
              key !== "CUE" &&
              !["NOMBRE", "APELLIDO", "CORREO_INSTITUCIONAL", "TELEFONO", "CARGO"].includes(key),
          )
          .reduce((obj, key) => {
            const lowerKey = key.toLowerCase()
            obj[lowerKey] = contact[key] || null
            return obj
          }, {}),
  ...
  try {
    ...
    if (action === "start" || action === "continue") {
      ...
      try {
        ...
        // Procesar los establecimientos uno por uno para evitar sobrecargar la base de datos
        for (const establishment of currentBatch) {
          try {
            const result = await processEstablishment(establishment, contacts)
          ...
          // Pequeña pausa entre cada establecimiento para no sobrecargar la API
